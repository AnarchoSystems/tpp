...
enum class TokKind
{
    Struct, Enum, LBrace, RBrace, Semi, Colon, LParen, RParen,
    Comma, LAngle, RAngle, KwOptional, KwList, KwString, KwInt,
    Ident, Eof
};
...
static std::vector<Token> tokenize_typedefs(const std::string &src)
{
    ...
    while (i < src.size()) {
        if (std::isspace(static_cast<unsigned char>(src[i]))) { advance(); continue; }
        int sline = line, scol = col;
        char c = src[i];
        if (c == '{') { tokens.push_back({TokKind::LBrace, "{", sline, scol}); advance(); continue; }
        if (c == '}') { tokens.push_back({TokKind::RBrace, "}", sline, scol}); advance(); continue; }
        if (c == ';') { tokens.push_back({TokKind::Semi, ";", sline, scol}); advance(); continue; }
        if (c == ':') { tokens.push_back({TokKind::Colon, ":", sline, scol}); advance(); continue; }
        if (c == '(') { tokens.push_back({TokKind::LParen, "(", sline, scol}); advance(); continue; }
        if (c == ')') { tokens.push_back({TokKind::RParen, ")", sline, scol}); advance(); continue; }
        if (c == ',') { tokens.push_back({TokKind::Comma, ",", sline, scol}); advance(); continue; }
        if (c == '<') { tokens.push_back({TokKind::LAngle, "<", sline, scol}); advance(); continue; }
        if (c == '>') { tokens.push_back({TokKind::RAngle, ">", sline, scol}); advance(); continue; }
        if (std::isalpha(static_cast<unsigned char>(c)) || c == '_') {
            ...
            if (word == "struct") k = TokKind::Struct;
            else if (word == "enum") k = TokKind::Enum;
            else if (word == "optional") k = TokKind::KwOptional;
            else if (word == "list") k = TokKind::KwList;
            else if (word == "string") k = TokKind::KwString;
            else if (word == "int") k = TokKind::KwInt;
...
struct TypedefParser
{
    ...
    const Token &eat(TokKind k) {
        if (cur().kind != k) { ok = false; }
        return tokens[pos++];
    }
    bool at(TokKind k) const { return cur().kind == k; }

    TypeRef parseTypeRef() {
        if (at(TokKind::KwString)) { eat(TokKind::KwString); return StringType{}; }
        if (at(TokKind::KwInt)) { eat(TokKind::KwInt); return IntType{}; }
        if (at(TokKind::KwOptional)) {
            eat(TokKind::KwOptional); eat(TokKind::LAngle);
            auto inner = parseTypeRef();
            eat(TokKind::RAngle);
            return std::make_shared<OptionalType>(OptionalType{inner});
        }
        if (at(TokKind::KwList)) {
            eat(TokKind::KwList); eat(TokKind::LAngle);
    ...
    void parseStruct() {
        eat(TokKind::Struct);
        auto &nameTok = cur(); eat(TokKind::Ident);
        StructDef sd; sd.name = nameTok.text;
        eat(TokKind::LBrace);
        while (!at(TokKind::RBrace) && !at(TokKind::Eof)) {
            auto &fnameTok = cur(); eat(TokKind::Ident);
            eat(TokKind::Colon);
        ...
        eat(TokKind::RBrace);
        if (at(TokKind::Semi)) eat(TokKind::Semi);
    ...
    void parseEnum() {
        eat(TokKind::Enum);
        auto &nameTok = cur(); eat(TokKind::Ident);
        EnumDef ed; ed.name = nameTok.text;
        eat(TokKind::LBrace);
        while (!at(TokKind::RBrace) && !at(TokKind::Eof)) {
            auto &tagTok = cur(); eat(TokKind::Ident);
            VariantDef vd; vd.tag = tagTok.text;
            if (at(TokKind::LParen)) {
                eat(TokKind::LParen);
                vd.payload = parseTypeRef();
                eat(TokKind::RParen);
            }
            if (at(TokKind::Comma)) eat(TokKind::Comma);
    ...
    bool validateTypes() {
        ...
        while (p < tokens.size() && tokens[p].kind != TokKind::Eof) {
            if (tokens[p].kind == TokKind::Struct) {
                skip(); skip(); skip(); // struct name {
                while (tokens[p].kind != TokKind::RBrace && tokens[p].kind != TokKind::Eof) {
                    skip(); skip(); // field_name :
                    if (!validateTypeFromTokens()) allOk = false;
                    skip(); // ;
                }
                skip(); // }
                if (p < tokens.size() && tokens[p].kind == TokKind::Semi) skip();
            } else if (tokens[p].kind == TokKind::Enum) {
                skip(); skip(); skip(); // enum name {
                while (tokens[p].kind != TokKind::RBrace && tokens[p].kind != TokKind::Eof) {
                    skip(); // tag
                    ...
                    if (tokens[p].kind == TokKind::Comma) skip();
                }
                skip(); // }
                if (p < tokens.size() && tokens[p].kind == TokKind::Semi) skip();
            } else {
    ...
    void parse() {
        while (!at(TokKind::Eof)) {
            if (at(TokKind::Struct)) parseStruct();
            else if (at(TokKind::Enum)) parseEnum();
            else { ok = false; pos++; }
        }
    }
};
...
static std::vector<Segment> splitAtDelimiters(const std::string &src)
{
...
static Expression parseExpression(const std::string &s) {
...
{
    Expr, For, EndFor, If, Else, Endif, Switch, Case, EndCase, EndSwitch
};
...
static DirectiveInfo classifyDirective(const std::string &s) {
    std::string t = trim(s);
    DirectiveInfo info;

    if (t == "else") { info.kind = DirectiveKind::Else; return info; }
    if (t == "endif") { info.kind = DirectiveKind::Endif; return info; }
    if (t == "end for") { info.kind = DirectiveKind::EndFor; return info; }
    if (t == "end switch") { info.kind = DirectiveKind::EndSwitch; return info; }
    if (startsWith(t, "end case")) { info.kind = DirectiveKind::EndCase; return info; }
    ...
    if (startsWith(t, "for ")) {
        ...
        if (!optPart.empty()) {
            ...
            while (p < optPart.size()) {
                while (p < optPart.size() && optPart[p] == ' ') ++p;
                if (p >= optPart.size()) break;
                size_t keyStart = p;
                while (p < optPart.size() && optPart[p] != '=') ++p;
                std::string key = trim(optPart.substr(keyStart, p - keyStart));
                if (p < optPart.size()) ++p;
                std::string val = parseStringLiteral(optPart, p);
                if (key == "sep") info.sep = val;
                else if (key == "followedBy") info.followedBy = val;
                while (p < optPart.size() && optPart[p] == ' ') ++p;
            }
        }
        return info;
...
static std::vector<TemplateLine> parseTemplateLines(const std::string &body) {
    ...
    for (auto &raw : rawLines) {
        TemplateLine tl;
        auto segs = splitAtDelimiters(raw);
        bool hasText = false;
        ...
        size_t ws = 0;
        while (ws < raw.size() && (raw[ws] == ' ' || raw[ws] == '\t')) ws++;
        tl.indent = (int)ws;
...
// Forward declarations for inline parser
struct InlineParser {
    ...
    std::vector<ASTNode> parse() {
        ...
        while (pos < segs.size()) {
            auto &s = segs[pos];
            if (!s.isDirective) {
                if (!s.text.empty()) nodes.push_back(TextNode{s.text});
                ++pos;
                ...
                while (pos < segs.size()) {
                    ...
                    if (cs.info.kind == DirectiveKind::EndSwitch) { ++pos; break; }
        ...
        while (pos < lines.size()) {
            ...
            if (tl.isBlockLine) {
                ...
                for (auto &s : tl.segments) {
                    if (s.isDirective && isStructuralDirective(s.info.kind)) { hasInline = true; break; }
                }
                if (hasInline) {
                    InlineParser ip{tl.segments};
                    auto inlineNodes = ip.parse();
                    for (auto &n : inlineNodes) nodes.push_back(std::move(n));
                    nodes.push_back(TextNode{"\n"});
                } else {
                    for (auto &seg : tl.segments) {
                        if (!seg.isDirective) {
                            if (!seg.text.empty()) nodes.push_back(TextNode{seg.text});
                        } else {
...
// Parse a type expression from a template param string like "list<String>", "optional<int>", "string"
static TypeRef parseParamType(const std::string &s) {
    std::string t = trim(s);
    if (t == "string" || t == "String") return StringType{};
    if (t == "int" || t == "Int") return IntType{};
...
static bool parseOneTemplate(const std::string &text, size_t &pos,
    ...
    if (!startsWith(firstLine, "template ")) return false;
    ...
    if (!trim(paramsStr).empty()) {
        ...
        for (char c : paramsStr) {
            ...
            else if (c == ',' && depth == 0) { paramParts.push_back(current); current.clear(); }
            else { current += c; }
        }
                       ...
                       std::vector<Diagnostic> &diagnostics) const noexcept
{
    try {
        ...
        while (pos < templateString.size()) {
            ...
            if (pos >= templateString.size()) break;
                                  ...
                                  std::vector<Diagnostic> &diagnostics) const noexcept
{
    try {
        ...
        Diagnostic d;
        d.range = {{0, 0}, {0, 0}};
        d.message = "function '" + functionName + "' not found";
        d.severity = DiagnosticSeverity::Error;
...
struct RenderContext {
    ...
    std::string jsonToString(const nlohmann::json &val) const {
        ...
        if (val.is_number_float()) {
            ...
            if (d == std::floor(d)) return std::to_string((int64_t)d);
