...
using StringType = compiler::StringType;
using IntType = compiler::IntType;
using BoolType = compiler::BoolType;
...
static std::vector<const tpp::FieldDef *> fieldsOf(const TypeRef &type, const WorkspaceProject &project) {
    return std::visit([&](auto &&arg) -> std::vector<const tpp::FieldDef *> {
        ...
        if constexpr (std::is_same_v<T, StringType> || std::is_same_v<T, IntType> || std::is_same_v<T, BoolType>) {
            ...
            static_assert(always_false_v<T>, "Unhandled TypeRef alternative in fieldsOf");
        }
    },
                      type);
}
    ...
    for (const auto &node : nodes) {
        bool found = std::visit([&](auto &&arg) -> bool {
            ...
            if constexpr (std::is_same_v<T, std::shared_ptr<IndentNode>>) {
                ...
                if (arg->sourceRange.start.line > line) {
                    return false;
                }
                ...
                if (walkForScope(arg->thenBody, line, character, scope, project)) {
                    return true;
                }
                ...
                for (const auto &c : arg->cases) {
                    if (walkForScope(c.body, line, character, scope, project)) {
                        return true;
                    }
                }
                if (arg->defaultCase) {
                ...
                return false;
            } else if constexpr (std::is_same_v<T, compiler::TextNode> ||
                                 std::is_same_v<T, compiler::AlignmentCellNode> ||
                                 std::is_same_v<T, compiler::CommentNode> ||
                                 std::is_same_v<T, compiler::InterpolationNode>) {
                return false;
            } else if constexpr (std::is_same_v<T, std::shared_ptr<compiler::FunctionCallNode>> ||
                                 std::is_same_v<T, std::shared_ptr<compiler::RenderViaNode>>) {
                return false;
            } else {
                static_assert(always_false_v<T>, "Unhandled ASTNode alternative in walkForScope");
            }
        },
                                node);
        if (found) {
...
static std::string identBeforeDot(const std::string &content, int line, int character) {
    ...
    if (dotPos == std::string::npos || dotPos == 0 || content[dotPos] != '.') {
        return {};
    }
...
static nlohmann::json makeItem(const std::string &label, int kind,
    ...
    nlohmann::json item = {{"label", label}, {"kind", kind}, {"sortText", std::to_string(priority) + "." + label}};
    if (!detail.empty()) {
        item["detail"] = detail;
    }
    return item;
