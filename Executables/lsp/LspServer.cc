...
nlohmann::json LspServer::handle(const std::string &rawMessage) {
    nlohmann::json msg;
    try {
        msg = nlohmann::json::parse(rawMessage);
    } catch (...) {
        return makeError(nullptr, -32700, "Parse error");
    }
...
nlohmann::json LspServer::onInitialize(const nlohmann::json &params) {
    ...
    return {
        {"capabilities", {{"textDocumentSync", 1}, // Full sync
                          {"semanticTokensProvider", {{"legend", {{"tokenTypes", lsp::tokenTypeLegend()}, {"tokenModifiers", nlohmann::json::array()}}}, {"full", true}}},
...
void LspServer::onInitialized(const nlohmann::json &, std::vector<nlohmann::json> &outNotifs) {
    ...
    nlohmann::json reg = {
        ...
        {"jsonrpc", "2.0"},
        {"params", {{"registrations", nlohmann::json::array({{{"id", "tpp-config-watcher"}, {"method", "workspace/didChangeWatchedFiles"}, {"registerOptions", {{"watchers", nlohmann::json::array({{{"globPattern", "**/tpp-config.json"}}})}}}}})}}}};
...
static void findConfigsRecursive(const fs::path &dir,
                                 ...
                                 int depth = 0) {
    if (depth > 8) {
        return; // guard against deep trees
    }
    std::error_code ec;
    for (const auto &entry : fs::directory_iterator(dir, ec)) {
        if (!ec && entry.is_directory(ec) && !ec) {
            ...
            if (name == "node_modules" || name == "build") {
                continue;
            }
...
nlohmann::json LspServer::onSemanticTokensFull(const nlohmann::json &params) {
    ...
    WorkspaceProject *proj = projectFor(uri);
    if (proj) {
        return computeSemanticTokens(uri, *proj);
    }
