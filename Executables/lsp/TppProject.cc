...
WorkspaceProject::WorkspaceProject(fs::path configPath)
    : configPath_(std::move(configPath)), root_(configPath_.parent_path()) {
    reloadConfig();
}
...
bool WorkspaceProject::reloadConfig() {
    ...
    for (const auto &p : typeFiles_) {
        uris_.insert(pathToUri(p));
        typeUris_.insert(pathToUri(p));
    }
    for (const auto &p : templateFiles_) {
        uris_.insert(pathToUri(p));
    }
    for (const auto &p : policyFiles_) {
        uris_.insert(pathToUri(p));
        policyUris_.insert(pathToUri(p));
    }
