...
@else@
@if ctx.needsStatic@static @end if@std::string @ctx.functionPrefix@@fn.name@(@for param in fn.params | sep=", "@[[maybe_unused]] @cpp_arg_type(param.type)@ @param.name@@end for@) {
    tpp::Writer _writer;
    @for instr in fn.body@
    @emit_instr(instr)@
    @end for@
