...
struct @s.name@
{
    ...
    @end for@
    friend void from_json(const nlohmann::json& j, @s.name@& v)
    {
        @for field in s.fields@
        @if field.innerType@
    ...
    friend void to_json(nlohmann::json& j, const @s.name@& v)
    {
        j = nlohmann::json{};
        @for field in s.fields@
        @if field.isOptional@
...
struct @e.name@
{
    using Value = std::variant<@for variant in e.variants | sep=", "@@if not variant.payloadType@@e.name@_@variant.tag@@else@@variant.payloadType@@endif@@end for@>;
    Value value;
    friend void from_json(const nlohmann::json& j, @e.name@& v)
    {
        @for variant in e.variants | sep="\n        else "@
        if (j.contains("@variant.tag@")) @if not variant.payloadType@v.value = @e.name@_@variant.tag@{};@else@v.value = j["@variant.tag@"].get<@variant.payloadType@>();@endif@
        @end for@
    }
    friend void to_json(nlohmann::json& j, const @e.name@& v)
    {
        const char* _tags[] = {@for variant in e.variants | sep=", "@"@variant.tag@"@end for@};
        std::visit([&](const auto& arg) {
            j = nlohmann::json::object();
            j[_tags[v.value.index()]] = nlohmann::json(arg);
        }, v.value);
    }
};
@end case@
