...
namespace codegen
{
    ...
    inline RenderTypeKind typeKindToContext(const tpp::TypeKind &t)
    {
        ...
        case 0:
        {
            result.value.emplace<0>();
            break;
        }
        case 1:
        {
            result.value.emplace<1>();
            break;
        }
        case 2:
        {
            result.value.emplace<2>();
            break;
        }
        case 3:
        {
            result.value.emplace<3>(qualifyPublicIRTypeName(std::get<3>(t.value)));
            break;
        }
        case 4:
        {
            result.value.emplace<4>(std::make_unique<RenderTypeKind>(typeKindToContext(*std::get<4>(t.value))));
            break;
        }
        case 5:
        {
            result.value.emplace<5>(std::make_unique<RenderTypeKind>(typeKindToContext(*std::get<5>(t.value))));
            break;
        }
        default:
        {
        ...
        bool shadowed = false)
    {
        ...
        if constexpr (std::is_same_v<T, tpp::EmitExprInstr>) {
            ...
            return false;
        } else if constexpr (std::is_same_v<T, tpp::EmitInstr>) {
            return false;
            ...
            static_assert(always_false_v<T>, "Unhandled Instruction alternative in instructionUsesBinding");
        } },
                          instr.value);
    }
        ...
        const ConvertConfig &cfg)
    {
        ...
        if constexpr (std::is_same_v<T, tpp::EmitInstr>) {
            ...
            static_assert(always_false_v<T>, "Unhandled Instruction alternative in convertInstruction");
        } },
                          instr.value);
    }
    ...
    inline std::vector<PolicyInfo> buildPolicyContext(const tpp::IR &ir)
    {
        ...
        for (const auto &pol : ir.policies)
        {
            ...
            {
                reqArr.push_back({stringLiteral(r.regex),
