...
static bool parsePolicy(const nlohmann::json &j,
    ...
    if (j.contains("length")) {
        ...
        if (lj.contains("min")) {
            if (!lj.at("min").is_number_integer()) {
                error = "[policy " + tag + "] 'length.min' must be an integer";
                return false;
            }
        ...
        if (lj.contains("max")) {
            if (!lj.at("max").is_number_integer()) {
                error = "[policy " + tag + "] 'length.max' must be an integer";
                return false;
            }
    ...
    if (j.contains("reject-if")) {
        ...
        if (rj.contains("message")) {
            ri.message = rj.at("message").get<std::string>();
        }
    ...
    auto parseRegexSteps = [&](const std::string &fieldName,
        ...
        for (const auto &item : arr) {
            ...
            if (item.contains("replace")) {
                step.replace = item.at("replace").get<std::string>();
            }
            step.compiled_rx = std::regex(step.regex);
            if (!step.replace.empty()) {
                step.compiled_replace = precompile_replace(step.replace);
            }
    ...
    if (!parseRegexSteps("require", pol.require)) {
        return false;
    }
    if (!parseRegexSteps("output-filter", pol.outputFilter)) {
        return false;
    }
