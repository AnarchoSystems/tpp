...
static void collectFoldNode(std::vector<ToolingFoldingRange> &out,
    ...
    std::visit([&](auto &&arg) {
        ...
        if constexpr (std::is_same_v<T, compiler::TextNode> ||
                      std::is_same_v<T, compiler::AlignmentCellNode> ||
                      std::is_same_v<T, compiler::InterpolationNode>) {
            return;
            ...
            static_assert(always_false_v<T>, "Unhandled ASTNode alternative in collectFoldNode");
        }
    },
               node);
}
...
static TypeSourceTokenKind convertTokenKind(TokKind kind) {
    ...
    case TokKind::Struct:
    {
        return TypeSourceTokenKind::Struct;
    }
    case TokKind::Enum:
    {
        return TypeSourceTokenKind::Enum;
    }
    case TokKind::LBrace:
    {
        return TypeSourceTokenKind::LBrace;
    }
    case TokKind::RBrace:
    {
        return TypeSourceTokenKind::RBrace;
    }
    case TokKind::Semi:
    {
        return TypeSourceTokenKind::Semi;
    }
    case TokKind::Colon:
    {
        return TypeSourceTokenKind::Colon;
    }
    case TokKind::LParen:
    {
        return TypeSourceTokenKind::LParen;
    }
    case TokKind::RParen:
    {
        return TypeSourceTokenKind::RParen;
    }
    case TokKind::Comma:
    {
        return TypeSourceTokenKind::Comma;
    }
    case TokKind::LAngle:
    {
        return TypeSourceTokenKind::LAngle;
    }
    case TokKind::RAngle:
    {
        return TypeSourceTokenKind::RAngle;
    }
    case TokKind::KwOptional:
    {
        return TypeSourceTokenKind::KwOptional;
    }
    case TokKind::KwList:
    {
        return TypeSourceTokenKind::KwList;
    }
    case TokKind::KwString:
    {
        return TypeSourceTokenKind::KwString;
    }
    case TokKind::KwInt:
    {
        return TypeSourceTokenKind::KwInt;
    }
    case TokKind::KwBool:
    {
        return TypeSourceTokenKind::KwBool;
    }
    case TokKind::Ident:
    {
        return TypeSourceTokenKind::Ident;
    }
    case TokKind::Comment:
    {
        return TypeSourceTokenKind::Comment;
    }
    case TokKind::DocComment:
    {
        return TypeSourceTokenKind::DocComment;
    }
    case TokKind::Eof:
    {
...
std::vector<TypeSourceToken> tokenizeTypeSource(const std::string &src) {
    ...
    for (const auto &token : tokens) {
        result.push_back({convertTokenKind(token.kind),
                          token.text,
...
std::vector<TemplateToken> tokenizeTemplateSource(const std::string &src) {
    ...
    while (lineStart <= src.size()) {
        ...
        for (const auto &segment : segments) {
            result.push_back({segment.isDirective,
                              segment.text,
...
std::vector<TemplateDirectiveRange> extractTemplateDirectiveRanges(const std::string &bodyText,
    ...
    for (size_t lineIndex = 0; lineIndex < templateLines.size(); ++lineIndex) {
        ...
        for (const auto &segment : line.segments) {
            ...
            ranges.push_back({{{static_cast<int>(bodyStartLine + lineIndex), segment.startCol - 1},
