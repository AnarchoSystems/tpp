...
inline bool operator==(const PolicyLength &a, const PolicyLength &b) { return a.min == b.min && a.max == b.max; }
inline bool operator==(const PolicyRejectIf &a, const PolicyRejectIf &b) { return a.regex == b.regex && a.message == b.message; }
inline bool operator==(const PolicyRegexStep &a, const PolicyRegexStep &b) { return a.regex == b.regex && a.replace == b.replace; }
inline bool operator==(const PolicyReplacement &a, const PolicyReplacement &b) { return a.find == b.find && a.replace == b.replace; }
...
inline void from_json(const nlohmann::json &j, PolicyLength &v) {
    if (j.contains("min")) {
        v.min = j.at("min").get<int>();
    }
    if (j.contains("max")) {
        v.max = j.at("max").get<int>();
    }
}
inline void to_json(nlohmann::json &j, const PolicyRejectIf &v) { j = {{"regex", v.regex}, {"message", v.message}}; }
...
inline void to_json(nlohmann::json &j, const PolicyRegexStep &v) { j = {{"regex", v.regex}, {"replace", v.replace}}; }
inline void from_json(const nlohmann::json &j, PolicyRegexStep &v) {
    ...
    v.compiled_rx = std::regex(v.regex);
    if (!v.replace.empty()) {
        v.compiled_replace = tpp::precompile_replace(v.replace);
    }
}
inline void to_json(nlohmann::json &j, const PolicyReplacement &v) { j = {{"find", v.find}, {"replace", v.replace}}; }
...
inline void to_json(nlohmann::json &j, const Policy &v) {
    ...
    if (v.rejectIf) {
        j["reject-if"] = *v.rejectIf;
    }
    if (!v.require.empty()) {
        j["require"] = v.require;
    }
    if (!v.replacements.empty()) {
        j["replacements"] = v.replacements;
    }
    if (!v.outputFilter.empty()) {
        j["output-filter"] = v.outputFilter;
    }
}
inline void from_json(const nlohmann::json &j, Policy &v) {
    j.at("tag").get_to(v.tag);
    if (j.contains("length")) {
        v.length = j.at("length").get<PolicyLength>();
    }
    if (j.contains("reject-if")) {
        v.rejectIf = j.at("reject-if").get<PolicyRejectIf>();
    }
    if (j.contains("require")) {
        v.require = j.at("require").get<std::vector<PolicyRegexStep>>();
    }
    if (j.contains("replacements")) {
        v.replacements = j.at("replacements").get<std::vector<PolicyReplacement>>();
    }
    if (j.contains("output-filter")) {
        v.outputFilter = j.at("output-filter").get<std::vector<PolicyRegexStep>>();
    }
}
