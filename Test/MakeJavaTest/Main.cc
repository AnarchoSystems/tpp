...
static std::string typeKindToJavaType(const tpp::TypeKind &t, const std::string &namespaceName = "") {
    ...
    case 3:
    {
        return qualifyJavaTypeName(std::get<3>(t.value), namespaceName); // Named
    }
    case 4:
    {
        return "java.util.List<" + typeKindToJavaBoxedType(*std::get<4>(t.value), namespaceName) + ">"; // List
    }
    case 5:
    {
        return typeKindToJavaType(*std::get<5>(t.value), namespaceName); // Optional
    }
    default:
    {
        return "Object";
...
static std::string typeKindToJavaBoxedType(const tpp::TypeKind &t, const std::string &namespaceName) {
    ...
    case 5:
    {
        return typeKindToJavaBoxedType(*std::get<5>(t.value), namespaceName); // Optional
    }
    default:
    {
        return "Object";
...
static std::string buildParseLine(const std::string &name, const tpp::TypeKind &type,
    ...
    case 4: { // List
        ...
        case 3:
        {
            elemExpr = qualifyJavaTypeName(std::get<3>(elemType.value), namespaceName) + ".fromJson(_arr.getJSONObject(_i))";
            break;
        }
        default:
        {
        ...
        return jt + " " + name + " = new java.util.ArrayList<>(); "
                                 "{ org.json.JSONArray _arr = new org.json.JSONArray(" +
               jsonLiteral + "); "
