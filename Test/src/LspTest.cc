...
//
// All suites are parameterised from runtime filesystem discovery under TestCases/.
// Case expectations are normalized through TestUtils, which prefers test-case.json
// and falls back to the legacy per-file layout while migration is in progress.
//
// Suites:
//   LspPreviewTest      — tpp/renderPreview matches normalized case expectations
//   LspDiagnosticsTest  — publishDiagnostics matches normalized case expectations
...
TEST(LspCompletionTest, FieldAndTypeNameCompletions)
{
    ...
    const std::string templateText = readFileStr(templatePath);
    const std::string typesText = readFileStr(typesPath);
    const std::string templateUri = fileUri(templatePath);
    const std::string typesUri = fileUri(typesPath);

    tpp_test::LspClient client(TPP_LSP_EXE);
    client.initialize(fileUri(tcDir));
    client.didOpen(templateUri, templateText);
    client.didOpen(typesUri, typesText);

    const size_t fieldDotOffset = templateText.find("field.");
    ASSERT_NE(fieldDotOffset, std::string::npos);
    const auto [fieldLine, fieldCharacter] =
        lineAndCharacterAtOffset(templateText, fieldDotOffset + std::string("field.").size());

    const auto fieldCompletion = client.request(
        "textDocument/completion",
        {{"textDocument", {{"uri", templateUri}}},
         {"position", {{"line", fieldLine}, {"character", fieldCharacter}}}});
    ASSERT_TRUE(fieldCompletion.is_array()) << fieldCompletion.dump();
    EXPECT_TRUE(hasCompletionLabel(fieldCompletion, "name"));
    EXPECT_TRUE(hasCompletionLabel(fieldCompletion, "type"));
    EXPECT_TRUE(hasCompletionLabel(fieldCompletion, "initialValue"));

    const auto typeCompletion = client.request(
        "textDocument/completion",
        {{"textDocument", {{"uri", typesUri}}},
         {"position", {{"line", 0}, {"character", 0}}}});
    ASSERT_TRUE(typeCompletion.is_array()) << typeCompletion.dump();
    EXPECT_TRUE(hasCompletionLabel(typeCompletion, "Field"));
    EXPECT_TRUE(hasCompletionLabel(typeCompletion, "TypeDef"));
}

// ── LspTokenTest ─────────────────────────────────────────────────────────────
// Parameterised on LspTokenSpec discovered from normalized test-case metadata.
